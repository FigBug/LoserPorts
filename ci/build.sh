...
if [ $OS = "mac" ]; then
  ...
  mv *.vst3 VST3
  mv *.component AU
  zip -r All_Loser_Mac.zip AU VST VST3

  if [ "$BRANCH" = "release" ]; then
    curl -F "files=@All_Loser_Mac.zip" "https://socalabs.com/files/set.php?key=$APIKEY"
  fi
elif [ $OS = "linux" ]; then
  cd "$ROOT/ci/bin"
  ...
  zip -r All_Loser_Linux.zip VST VST3 LV2

  if [ "$BRANCH" = "release" ]; then
    curl -F "files=@All_Loser_Linux.zip" "https://socalabs.com/files/set.php?key=$APIKEY"  
  fi
else
  cd "$ROOT/ci/bin"

  7z a All_Loser_Win.zip VST VST_32 VST3 VST3_32

  if [ "$BRANCH" = "release" ]; then
    curl -F "files=@All_Loser_Win.zip" "https://socalabs.com/files/set.php?key=$APIKEY"
  fi
fi
